...
- A builder ([builder/manifestory](builder/manifestory))
- A provisioner ([provisioner/manifestory](provisioner/manifestory))
- A post-processor ([post-processor/manifestory](post-processor/manifestory))
- A data source ([datasource/manifestory](datasource/manifestory))
- Docs ([docs](docs))
...
Then source the built binary to the plugin path with `cp packer-plugin-manifestory ~/.packer.d/plugins/packer-plugin-manifestory`
...
This manifestory configures a [manually triggered plugin test action](/.github/workflows/test-plugin-example.yml).
...
To include plugin docs on Packer.io a global pre-hook has been added to the main manifestory .goreleaser.yml file, that if uncommented will generate and include a docs.zip file as part of the plugin release.
...
```json
{
   "title": "manifestory",
   "path": "manifestory",
   "repo": "hashicorp/packer-plugin-manifestory",
   "version": "latest",
   "sourceBranch": "main"
 }
```
...
This manifestory template is compatible with Packer >= v1.7.0
